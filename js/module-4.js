...
//    const userFriends = [...new Set(users.flatMap(user => user.friends))];
//   return userFriends;
// };
...
//    return [...new Set(users.filter(user=>user.isActive === true))]
// };
...
//     return [...new Set(users.filter(user=>user.isActive === false))]
// };
...
// const totalAveragePlaytimePerGame = players.reduce((total, player) => {const averagePlaytime = player.playtime/player.gamesPlayed;
...
// const getSortedFriends = users => users.flatMap(user => user.friends).filter((friend, index, array) => array.indexOf(friend) === index).sort();
